function [c_n]=fun_FFT2D_new(fun_xy,nn,mm,X,Y,NGauss)
...
% % [X] = reshape(x,numel(y),numel(x))';
% % [Y] = reshape(y,numel(y),numel(x))';
...
ZZ = interp2(X,Y,fun_xy,XX,YY,'spline');

% % frz = scatteredInterpolant(X(:),Y(:),fun_xy(:),'linear');
% % ZZ = frz(XX,YY);
...
% % mm_tmp = -floor(size(XX,1)/2):floor(size(XX,1)/2);
% %
% % if length(mm_tmp) == size(XX,1)+1
% %     mm_tmp = mm_tmp(2:end);
% % end
% %
% % ind_mm = zeros(size(mm));
% % for ii=1:length(mm)
% %    ind_mm(ii) = find(mm_tmp == mm(ii));
% % end
% %
% % nn_tmp = -floor(size(XX,2)/2):floor(size(XX,2)/2);
% %
% % if length(nn_tmp) == size(XX,2)+1
% %     nn_tmp = nn_tmp(2:end);
% % end
% %
% % ind_nn = zeros(size(nn));
% % for ii=1:length(nn)
% %    ind_nn(ii) = find(nn_tmp == nn(ii));
% % end
% %
% % c_nm = zeros(length(mm_tmp),length(nn_tmp));
% % for ii=1:length(nn_tmp)
% %
% %     theta_ii = YY(:,(ii));
% %     phi_ii = XX(1,(ii));
% %
% %     TT = exp(1i*(theta_ii*mm_tmp + phi_ii+nn_tmp(ii)));
% %
% %     c_ii = TT\ZZ(:,ii);
% %
% %     c_nm(:,ii) = c_ii;
